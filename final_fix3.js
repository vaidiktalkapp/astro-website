...
code = code.split('dynamicData').join('puja');

// Replace Rudrabhishek text with dynamic puja title
code = code.split('>Rudrabhishek Puja<').join(">{puja?.title || 'Puja'}<");
code = code.split('Rudrabhishek <span className="text-[#d97706]">Pooja</span>').join("{puja?.title || 'Puja'}");
code = code.split('>Rudrabhishek<').join(">{puja?.title || 'Puja'}<");
code = code.split('>The Divine Power of <br /><span className="text-[#d97706]">{puja?.title || \'Puja\'}</span><').join(">The Divine Power of <br /><span className=\"text-[#d97706]\">{puja?.title || 'Puja'}</span><");

// For images array in Carousel
code = code.split(
  `["/pooja/Rudraabhishek.webp", "https://static-poojan.astroarunpandit.org/poojan/prod/generic/unassigned/en/upload/7b0f1a39-cd6d-4380-ae41-ce9ea3ce8e72.jpg"]`
).join(
...
code = code.split(
  `<img src="/pooja/Rudraabhishek.webp" alt="Shiva Lingam"`
).join(
...
code = code.split(`href="#booking-section"`).join("href={`/book-a-puja/${slug}/checkout`}");
