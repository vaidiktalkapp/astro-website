...
const KUNDLI_HOUSES = [
  {
    ...
    rules: "Self, personality, physical body, overall life direction",
  },
  { house: "2nd House", rules: "Wealth, family, speech, accumulated assets" },
  {
    house: "3rd House",
    rules: "Courage, siblings, communication, short journeys",
  },
  {
    house: "4th House",
    rules: "Mother, home, happiness, property, emotional roots",
  },
  {
    house: "5th House",
    rules: "Intelligence, children, creativity, past life merit",
  },
  {
    house: "6th House",
    rules: "Enemies, health, debts, daily service and routine",
  },
  {
    house: "7th House",
    rules: "Marriage, partnerships, business relationships",
  },
  {
    house: "8th House",
    rules: "Longevity, transformation, inheritance, hidden matters",
  },
  {
    house: "9th House",
    rules: "Dharma, fortune, father, higher learning, spirituality",
  },
  {
    house: "10th House",
    rules: "Career, status, public life, authority and reputation",
  },
  {
    house: "11th House",
    rules: "Gains, desires, friends, elder siblings, income",
  },
  {
    house: "12th House",
...
const NAVAGRAHAS = [
  {
    planet: "Sun (Surya)",
    rules: "Soul, authority, father, health, government",
  },
  {
    planet: "Moon (Chandra)",
    rules: "Mind, emotions, mother, comfort, intuition",
  },
  {
    planet: "Mars (Mangal)",
    rules: "Energy, courage, property, siblings, conflict",
  },
  {
    planet: "Mercury (Budha)",
    rules: "Intelligence, communication, business, education",
  },
  {
    planet: "Jupiter (Guru)",
    rules: "Wisdom, fortune, children, spirituality, expansion",
  },
  {
    planet: "Venus (Shukra)",
    rules: "Love, beauty, luxury, marriage, arts and pleasure",
  },
  {
    planet: "Saturn (Shani)",
    rules: "Discipline, karma, delay, service, longevity",
  },
  {
    planet: "Rahu",
...
const KundliClient = () => {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      const isValid =
        ...
        lat !== null &&
        lat !== "" &&
        String(lat) !== "undefined" &&
        String(lat) !== "null" &&
        lon !== undefined &&
        lon !== null &&
        lon !== "" &&
        String(lon) !== "undefined" &&
        String(lon) !== "null";
  ...
  const handleGenerate = async (
    ...
    try {
      ...
      if (year > 2100 || year < 1800) {
        toast.error(
          `Invalid year: ${year}. Please enter a year between 1800 and 2100.`,
        );
        setLoading(false);
        return;
      }
      ...
      const cleanCoord = (val: any) => {
        const s = String(val);
        if (!val || s === "undefined" || s === "null" || s.trim() === "")
          return null;
      ...
      if (!cleanLat || !cleanLon) {
        toast.error(
          "Invalid coordinates. Please search and select your birth city again.",
        );
        kundliStorage.clearData();
        setLoading(false);
        return;
      ...
      if (response.success) {
        ...
        toast.error(
          response.message || "Calculation failed. Please check your details.",
        );
      }
    } catch (err: any) {
      console.error("Kundli Error:", err);
      toast.error(
        err.response?.data?.message || "Failed to connect to celestial bridge.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <GeoapifyContext apiKey="47b4a8afc7734a12bd28b482d3dbff76">
      <div
        className="min-h-screen py-12 px-4 sm:px-6 z-10 relative"
      ...
      >
        ...
        <div className="mx-auto kundli-wrap max-w-6xl">
          <AnimatePresence mode="wait">
            ...
            {loading && !result && (
              ...
              >
                ...
                <h1 className="text-3xl font-semibold text-gray-900 mb-2">
                  {t("kundli.synchronizing_with_stars")}
                </h1>
                <p className="text-gray-850 font-medium max-w-sm">
                  {t("kundli.re_aligning_the_celestial_coor")}
                </p>
              </motion.div>
            )}
            ...
            {!result ? (
              ...
              >
                ...
                <div className="space-y-12 mt-10 mb-14">
                  ...
                  <section className="border-t border-[#d6c89a]/50 pt-10">
                    <h2 className="text-3xl font-semibold text-gray-900 mb-4">
                      {t("kundli.what_is_a_janam_kundli")}
                    </h2>
                    <div className="space-y-4 text-gray-900 leading-relaxed text-[15px]">
                      <p>{t("kundli.a_janam_kundli_also_known_as_a")}</p>
                      <p>{t("kundli.the_word_kundli_is_derived_fro")}</p>
                      <p>{t("kundli.for_centuries_the_janam_kundli")}</p>
                    </div>
                  </section>
                  ...
                  <section className="border-t border-[#d6c89a]/50 pt-10">
                    <h2 className="text-3xl font-semibold text-gray-900 mb-4">
                      {t("kundli.the_12_houses_of_the_kundli")}
                    </h2>
                    <div className="space-y-4 text-gray-900 leading-relaxed text-[15px] mb-6">
                      <p>{t("kundli.the_kundli_is_divided_into_12")}</p>
                    </div>
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="border-b border-[#d6c89a]/60 bg-[#fdf6e3]/40">
                            <th className="p-4 text-[12px] font-black text-gray-850 uppercase tracking-wider">
                              {t("kundli.house")}
                            </th>
                            <th className="p-4 text-[12px] font-black text-gray-850 uppercase tracking-wider">
                              {t("kundli.area_of_life")}
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[#d6c89a]/30">
                          {KUNDLI_HOUSES.map((h, i) => (
                            ...
                            >
                              <td className="p-4 font-semibold text-gray-850 text-[14px] whitespace-nowrap">
                                {h.house}
                              </td>
                              <td className="p-4 text-[13px] text-gray-850 leading-relaxed">
                                {h.rules}
                              </td>
                            </tr>
                          ))}
                  ...
                  <section className="border-t border-[#d6c89a]/50 pt-10">
                    <h2 className="text-3xl font-semibold text-gray-900 mb-4">
                      {t("kundli.the_navagrahas_nine_planets_in")}
                    </h2>
                    <div className="space-y-4 text-gray-900 leading-relaxed text-[15px] mb-6">
                      <p>{t("kundli.vedic_astrology_recognises_nin")}</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {NAVAGRAHAS.map((g, i) => (
                        <div
                          key={i}
                          className="p-5 rounded-2xl border border-[#d6c89a]/60"
                        >
                          <p className="font-bold text-[#b8962e] text-[14px] mb-1.5">
                            {g.planet}
                          </p>
                  ...
                  <section className="border-t border-[#d6c89a]/50 pt-10">
                    <h2 className="text-3xl font-semibold text-gray-900 mb-4">
                      {t("kundli.how_to_read_your_janam_kundli")}
                    </h2>
                    <div className="space-y-4 text-gray-900 leading-relaxed text-[15px] mb-6">
                      <p>{t("kundli.reading_a_kundli_involves_unde")}</p>
                    </div>
                    <div className="space-y-3">
                      {[
                        ...
                        >
                          ...
                          <div>
                            <p className="font-semibold text-gray-850 text-[14px] mb-1">
                              {tip.title}
                            </p>
                ...
                <div
                  ref={formRef}
                  className="scroll-mt-8 border-t border-[#d6c89a]/50 pt-10"
                >
                  <div className="mb-8">
                    ...
                    <h2 className="text-3xl font-semibold text-gray-900 mb-2">
                      {t("kundli.generate_your_janam_kundli")}
                    </h2>
                  ...
                  <form
                    onSubmit={(e) => {
                      ...
                      if (isInvalidCoord) {
                        toast.error(
                          "Please search and select your birth city from the dropdown list to confirm location.",
                        );
                        return;
                      }
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2 pb-3 border-b border-[#d6c89a]">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[15px] font-semibold text-gray-850">
                          {t("kundli.enter_birth_details")}
                        </span>
                      </div>
                      {isAuthenticated && (
                        <button
                        ...
                        >
                          ...
                          {t("common.use_profile") || "Use My Profile"}
                        </button>
                      )}
                    </div>
                    ...
                    {/* Name */}
                    <div>
                      <label className="block text-[15px] font-semibold text-gray-850 mb-2.5">
                        {t("kundli.full_name")}
                      </label>
                      <input
                        type="text"
                      ...
                      <div className="relative kundli-geo z-[100]">
                          ...
                          placeSelect={(value: any) => {
                            if (value && value.properties) {
                              ...
                              if (
                                ...
                                value.properties.timezone.offset_STD_seconds !==
                                  undefined
                              ) {
                                selectedTzone =
                                  value.properties.timezone.offset_STD_seconds /
                        ...
                        {formData.place && !formData.lat && (
                          <p className="text-[11px] text-amber-600 font-semibold mt-1.5">
                            ⚠️ Please select your city from the dropdown to
                            confirm location.
                          </p>
                        )}
                      </div>
                    ...
                    <div className="pt-2">
                      <motion.button
                        whileHover={
                          !formData.lat || loading ? {} : { scale: 1.005 }
                        }
                        whileTap={
                          !formData.lat || loading ? {} : { scale: 0.995 }
                        }
                        type="submit"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-12 pt-10 border-t border-[#d6c89a]">
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-[14px] font-semibold text-gray-850">
                          {t("kundli.janam_kundli")}
                        </span>
                      </div>
                      <p className="text-[13px] text-gray-850 leading-relaxed">
                        {t("kundli.complete_birth_chart_with_all")}
                      </p>
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-[14px] font-semibold text-gray-850">
                          {t("kundli.planetary_positions")}
                        </span>
                      </div>
                      <p className="text-[13px] text-gray-850 leading-relaxed">
                        {t("kundli.graha_positions_dashas_and_tra")}
                      </p>
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-[14px] font-semibold text-gray-850">
                          {t("kundli.ascendant_nakshatra")}
                        </span>
                      </div>
                      <p className="text-[13px] text-gray-850 leading-relaxed">
                        {t("kundli.lagna_birth_star_and_rashi_det")}
                      </p>
                    </div>
                  </div>
                ...
                <Suspense
                  fallback={
                    <div className="flex items-center justify-center p-10">
                      <div className="w-10 h-10 border-4 border-orange-100 border-t-[#b8962e] rounded-full animate-spin" />
                    </div>
                  }
                >
                  <KundliResult
