...
const CountdownTimer = () => {
  const getSecondsLeft = () => {
    const now = Date.now();
    const stored = typeof window !== 'undefined' ? localStorage.getItem('dhan_laxmi_timer_start') : null;
    const start = stored ? parseInt(stored) : now;
    if (!stored && typeof window !== 'undefined') localStorage.setItem('dhan_laxmi_timer_start', String(now));
    const elapsed = Math.floor((now - start) / 1000);
    const cycle = 6 * 3600; // 6 hours
  ...
  return (
    <div className="flex items-center gap-3 my-3 bg-[#fff4e0] border border-[#fcd9a0] rounded-[10px] px-4 py-[10px] w-fit">
      <div className="text-[12px] font-bold text-[#b45309] uppercase tracking-wider">Offer ends in</div>
      <div className="flex gap-1.5 text-[#ea580c] font-black text-[18px]">
        <span className="bg-white px-2 py-0.5 rounded shadow-sm">{pad(h)}</span>:
        <span className="bg-white px-2 py-0.5 rounded shadow-sm">{pad(m)}</span>:
        <span className="bg-white px-2 py-0.5 rounded shadow-sm text-[#db2777]">{pad(s)}</span>
      </div>
    </div>
  );
};
...
export default function DhanLaxmiPujaPage() {
  ...
  useEffect(() => {
    const fetchPuja = async () => {
      try {
        ...
        const res = await axios.get(`${apiUrl}/pujas/dhan-laxmi-puja`);
  ...
  const defaultImages = ["/pooja/Money Attract Puja (Dhan Laxmi Puja).webp", "https://static-poojan.astroarunpandit.org/poojan/prod/generic/unassigned/en/upload/7b0f1a39-cd6d-4380-ae41-ce9ea3ce8e72.jpg"];
  const galleryImages = dynamicData?.gallery?.length > 0 ? dynamicData.gallery : defaultImages;
  const price = dynamicData?.discountedPrice || dynamicData?.price || 1999;
  const origPrice = Math.round(price * 1.28);

  const testimonials = [
    { name: "Priya Sharma", city: "New Delhi", review: "The puja was absolutely divine. The pandit was deeply knowledgeable and performed every ritual with precision. I joined via live video and felt immense spiritual energy. Highly recommend Vaidik Talk.", initial: "P" },
    { name: "Rajesh Gupta", city: "Mumbai", review: "Skeptical at first, but this completely changed my view of online pujas. The sankalp was taken in my name and gotra. The results were visible within a week.", initial: "R" },
    { name: "Anita Verma", city: "Bengaluru", review: "Booked this puja for financial stability in my business. The muhurat was perfectly auspicious. The difference was palpable. Will always trust Vaidik Talk.", initial: "A" },
    { name: "Vikram Nair", city: "Chennai", review: "My financial blockages cleared surprisingly fast after this puja. The pandits were very authentic.", initial: "V" },
    { name: "Kavita Joshi", city: "Ahmedabad", review: "Best puja I have ever booked online. The HD video, prasad delivery, and follow-up were all excellent.", initial: "K" },
  ];

  const steps = [
    { title: "Select your Puja package", desc: "Choose the package that best matches your intention and purpose." },
    { title: "Enter your sankalp details", desc: "Add your Name, Gotra & financial intention. The pandit will personalize the Sankalp." },
  ...
  const faqs = [
    { q: "Who should perform the Dhan Laxmi Puja?", a: "Anyone facing continuous financial instability, business losses, debts, or anyone who wishes to multiply their wealth and open new avenues of income." },
    { q: "How does this puja help with debts?", a: "The puja invokes the energy of Mahalakshmi and Kubera, working on a karmic level to dissolve financial blockages and attract opportunities that help you clear debts effectively." },
    { q: "Can I participate in the puja online?", a: "Absolutely. Our expert Pandits perform the sacred Sankalp using your name and gotra. You can join the live-streamed ceremony from the comfort of your home." },
    { q: "What is the best time to perform this puja?", a: "Days like Dhan Trayodashi, Purnima, Fridays, or during specific favorable planetary transits yield the most profound results." },
    { q: "Which deities are worshipped in this puja?", a: "The puja invokes Goddess Lakshmi (prosperity), Lord Kubera (wealth), and Vishnu Tatva to align positive energies for financial success." },
  ];
  ...
  return (
    <div className="w-full bg-white text-[#3a1216]" style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>
      ...
      <div className="max-w-[1140px] mx-auto px-5 pb-16">
          ...
          <div className="flex-1 min-w-[280px]">
            ...
            <div className="space-y-[8px] mb-4">
              ...
              <p className="flex items-center gap-2 text-[#3a1216] text-[14px] m-0 font-medium">
                ...
                Wealth-focused ritual on Auspicious Muhurat
              </p>
              <p className="flex items-center gap-2 text-[#3a1216] text-[14px] m-0 font-medium">
                ...
                Sankalp with your Name, Gotra &amp; Financial intention
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between my-3 gap-1 sm:gap-0 text-[15px] text-[#3a1216] font-medium">
              <span>9K+ devotees booked this puja</span>
              <span className="text-[#9c5c0f] font-bold text-[14px]">★ 4.9 (6.1K Reviews)</span>
            </div>
            <div className="flex items-end justify-between mt-5 pt-4 border-t border-[#e5e0d8]">
              ...
              <Link href="/book-a-puja/dhan-laxmi-puja/checkout" className="bg-[#d97706] hover:bg-[#b56003] text-white py-[13px] px-[30px] rounded-[10px] font-bold text-[16px] no-underline inline-block transition-colors mb-0">
                Book Puja
              </Link>
            </div>
          </div>
        ...
        <section className="py-12 border-t border-[#e5e0d8] mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {["Attracts wealth & financial stability", "Clears business & career blockages", "Brings peace, prosperity & happiness", "Removes debt & financial struggles", "Invokes Goddess Laxmi's divine blessings"].map((b, i) => (
        ...
        <section className="py-8 border-t border-[#e5e0d8]">
          ...
          <div className="flex gap-6 flex-wrap items-start">
            <img src="/pooja/Money Attract Puja (Dhan Laxmi Puja).webp" alt="Dhan Laxmi Puja" className="w-full max-w-[420px] rounded-[12px] object-cover aspect-[4/3]" />
            <div className="flex-1 min-w-[250px] space-y-4">
              <p className="m-0 text-[#3a1216] text-[16px] leading-[1.8]">
                The Money Attract Anushthan (Dhan Laxmi Puja) is a sacred and result-oriented Vedic ritual designed to activate your wealth energy and invite steady financial growth, success, and prosperity into your life.
              </p>
              <p className="m-0 text-[#3a1216] text-[16px] leading-[1.8]">
                As a powerful Wealth Puja and Puja for Prosperity, this sacred ritual is rooted in ancient scriptures. This Goddess Laxmi Puja helps release karmic debts, remove obstacles to abundance, and attract Lakshmi Tatva—the divine essence of wealth and fortune.
              </p>
              <p className="m-0 text-[#3a1216] text-[16px] leading-[1.8]">
                Whether you’re experiencing financial instability, facing repeated money-related challenges, or simply wish to grow your income and wealth potential, this puja offers a spiritually guided way to seek divine blessings.
              </p>
            </div>
          </div>
        </section>
            ...
            {packages.map((pkg, i) => {
              ...
              return (
                ...
                >
                  ...
                  <Link
                    href={`/book-a-puja/dhan-laxmi-puja/checkout?package=${pkg.name.toLowerCase()}&price=${pkg.price}`}
        ...
        <section className="py-8 border-t border-[#e5e0d8]">
          ...
          <p className="text-[#3a1216] m-0 mb-4 text-[16px] leading-[1.8]">
            Vaidik Talk is a dedicated puja platform connecting devotees with verified Pandits for authentic Vedic rituals. Every Dhan Laxmi puja is performed with a real Sankalp taken in your name and intention — so you can receive divine financial blessings from anywhere in India or abroad.
          </p>
        ...
        <section className="py-8 border-t border-[#e5e0d8]">
          <div className="flex flex-wrap gap-4 items-center justify-between border border-[#e5e0d8] rounded-[16px] p-5 bg-[#faf6ee]">
            ...
            <div className="text-center">
              <a href="https://wa.me/919818999037?text=Hi%2C+I+want+to+book+Dhan+Laxmi+Puja" target="_blank" rel="noopener noreferrer" className="inline-block bg-[#25d366] hover:bg-[#1da851] text-white py-[14px] px-[32px] rounded-[12px] font-bold text-[15px] no-underline transition-colors">
                Chat on WhatsApp
              </a>
      ...
      <MobileStickyBar price={price} slug="dhan-laxmi-puja" timerKey="dhan_laxmi_timer_start" />
    </div>
  );
}
