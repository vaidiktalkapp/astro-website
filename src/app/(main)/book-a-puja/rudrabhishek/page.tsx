...
export default function RudrabhishekPage() {
  ...
  const { formData, handleChange, handleSubmit, isProcessing } = usePujaBooking({
    title: 'Rudrabhishek',
    slug: 'rudrabhishek',
    amount: dynamicData?.discountedPrice || dynamicData?.price || 1599
  });

  useEffect(() => {
    import('axios').then(axios => {
      const url = (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/v1') + '/pujas/rudrabhishek';
    ...
    <div className="w-full bg-[#fcf5eb] font-sans">
      ...
      <div className="relative w-full min-h-[500px] md:min-h-[600px] py-12 md:py-0 flex items-center bg-black overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 pt-10">
          ...
          <h1 className="premium-serif text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-6 leading-tight max-w-3xl">
            Rudrabhishek <span className="text-[#f5d08b]">Pooja</span>
          </h1>
          ...
          >
            <Sparkles className="w-5 h-5" /> Book Now @ ₹1599
          </a>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          <div>
            ...
            <p className="text-gray-900 text-lg leading-relaxed mb-6">
              The Rudrabhishek Pooja is one of the most sacred and potent Vedic rituals dedicated to Lord Shiva in his fierce yet benevolent form—Rudra. As a significant Hindu Puja, It involves the ceremonial bathing (abhishek) of the Shiva Lingam with sacred offerings like water, milk, honey, curd, ghee, and belpatra, accompanied by powerful Vedic mantras from the Rudram Chamakam. Through Online Puja Services, devotees can Book Puja Online and participate in this divine pooja with spiritual devotion.
            </p>
            <p className="text-gray-800 text-lg leading-relaxed mb-6">
              This divine pooja is known to destroy negative karma, pacify planetary doshas (especially linked to Rahu, Ketu, and Shani), protect from misfortunes, and bless the devotee with inner strength, peace, and spiritual awakening.
            </p>
            <ul className="space-y-4 mt-8">
              {[
                "To remove negative energies, black magic, or evil eye",
                "To pacify malefic effects of planets like Shani, Rahu, Ketu, or weak Moon",
                "To heal health issues and chronic emotional/mental disturbances",
                "For protection during critical life transitions or astrological dasha periods",
                "To deepen spiritual connection with Lord Shiva and awaken inner peace"
              ].map((item, i) => (
      ...
      <div className="w-full py-16 md:py-20 px-6 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-800 text-base md:text-lg max-w-2xl mx-auto font-medium">Different offerings during Abhishek yield specific spiritual results based on your Sankalp.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { title: "Jal Abhishek", desc: "For peace and emotional cleansing", icon: <Droplet className="w-6 h-6" /> },
              { title: "Dugdha (Milk)", desc: "For purity and spiritual healing", icon: <Sparkles className="w-6 h-6" /> },
              { title: "Honey", desc: "For sweetness in relationships", icon: <Heart className="w-6 h-6" /> },
              { title: "Dahi (Curd)", desc: "For fertility and family well-being", icon: <Star className="w-6 h-6" /> },
              { title: "Ghee", desc: "For clarity, wealth, and removal of obstacles", icon: <Flame className="w-6 h-6" /> },
              { title: "Bhasma & Bilva Leaves", desc: "For deep surrender and awakening Shiva consciousness", icon: <ShieldCheck className="w-6 h-6" /> }
            ].map((offering, idx) => (
              <div key={idx} className="bg-white border border-[#f0ddc0] p-6 rounded-2xl hover:border-[#d4af37]/60 hover:shadow-lg transition-all duration-300 text-center group relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-[#d4af37]/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="w-14 h-14 mx-auto bg-[#fcf5eb] text-[#d4af37] border border-[#f0ddc0] rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform group-hover:bg-[#d4af37] group-hover:text-white shadow-sm">
                  {offering.icon}
                </div>
                <h3 className="text-lg font-bold text-[#5c1a1f] mb-2 tracking-wide">{offering.title}</h3>
                <p className="text-gray-800 text-sm md:text-base leading-relaxed font-medium">{offering.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="py-20 px-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <div>
          ...
          <div className="space-y-8 relative before:absolute before:inset-0 before:ml-6 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-[#d4af37] before:to-transparent">
            {[
              "Book your pooja seamlessly through our platform",
              "Share your personal details (name, gotra, intentions) to be included in the Sankalp by our Pandit",
              "On the pooja day, be present (in person or online), cover your head respectfully, and surrender with devotion",
              "Listen as the Rudram is chanted and the sacred Abhishek is performed on the Shivlinga, invoking Lord Rudra’s fierce grace and divine compassion",
              "Receive blessings and personalized spiritual remedies after the pooja"
            ].map((step, idx) => (
        ...
        <div>
          ...
          <ul className="space-y-3 mb-10">
            {[
              ...
              "Your Sankalp is honored and personalized, ensuring maximum spiritual impact",
              "Seamless booking, live participation, and ritual guidance included",
              "Receive visual Kundali insights and remedies based on your planetary alignment",
              "Backed by over 40+ years of spiritual and Vedic experience"
            ].map((item, i) => (
              <li key={i} className="flex items-start gap-3 text-gray-800 font-medium text-sm md:text-base">
            ...
            <div className="w-full lg:w-2/3 bg-white p-6 md:p-10 rounded-2xl shadow-xl border border-[#e8d8c0]/50 relative overflow-hidden">
              ...
              <form id="booking-form" onSubmit={handleSubmit} className="space-y-5 md:space-y-6 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-800 mb-1.5">Gotra (Optional)</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                  <div>
                    <label className="block text-sm font-semibold text-gray-800 mb-1.5">Phone No. *</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-800 mb-1.5">Preferred Pooja Date *</label>
                    <input required type="date" name="date" value={formData.date} onChange={handleChange} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-[#d4af37] focus:border-[#d4af37] outline-none transition-all text-gray-800 bg-gray-50/50" />
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-semibold text-gray-800 mb-1.5">Additional Message / Intentions</label>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 shadow-[0_-10px_25px_rgba(0,0,0,0.1)] z-[100] flex justify-between items-center">
        <div className="flex flex-col">
          <span className="text-[10px] text-gray-800 font-bold uppercase tracking-wider mb-0.5">Total Offering</span>
