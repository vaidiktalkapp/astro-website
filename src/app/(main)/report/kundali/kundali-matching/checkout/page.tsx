...
import { Check, Clock, Plus, X, Users, ShieldCheck, ChevronDown, CheckSquare, Square, MapPin, Loader2, User } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  const [boyLocationSuggestions, setBoyLocationSuggestions] = useState<any[]>([]);
  const [isSearchingBoyLocation, setIsSearchingBoyLocation] = useState(false);
  ...
  const [girlLocationSuggestions, setGirlLocationSuggestions] = useState<any[]>([]);
  const [isSearchingGirlLocation, setIsSearchingGirlLocation] = useState(false);
  ...
  // Debounce Location Search for Boy
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (boyData.pob && boyData.pob.length > 2 && showBoySuggestions) {
        setIsSearchingBoyLocation(true);
        try {
          const res = await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(boyData.pob)}&limit=5`);
          const data = await res.json();
          setBoyLocationSuggestions(data.features || []);
        } catch (err) {
          console.error(err);
        } finally {
          setIsSearchingBoyLocation(false);
        }
      } else {
        setBoyLocationSuggestions([]);
      }
    }, 500);
    return () => clearTimeout(delayDebounceFn);
  }, [boyData.pob, showBoySuggestions]);

  // Debounce Location Search for Girl
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      if (girlData.pob && girlData.pob.length > 2 && showGirlSuggestions) {
        setIsSearchingGirlLocation(true);
        try {
          const res = await fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(girlData.pob)}&limit=5`);
          const data = await res.json();
          setGirlLocationSuggestions(data.features || []);
        } catch (err) {
          console.error(err);
        } finally {
          setIsSearchingGirlLocation(false);
        }
      } else {
        setGirlLocationSuggestions([]);
      }
    }, 500);
    return () => clearTimeout(delayDebounceFn);
  }, [girlData.pob, showGirlSuggestions]);


  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    // Boy's details go into standard ReportBooking fields.
    // Girl's details go into partnerDetails.
    const overrideData = {
      name: boyData.name,
      gender: 'male',
      dob: boyData.dob,
      tob: boyUnknownTime ? 'Unknown' : boyData.tob,
      pob: boyData.pob,
      ...
      partnerDetails: {
        ...
        gender: 'female',
        dob: girlData.dob,
        tob: girlUnknownTime ? 'Unknown' : girlData.tob,
        pob: girlData.pob,
        country: girlData.country || 'India',
        state: girlData.state,
  ...
  const renderPersonForm = (
    ...
    <div className="mb-10">
      <div className="flex items-center gap-2 mb-5 pb-3 border-b border-[#ebdcc7]">
        <div className="w-8 h-8 rounded-full bg-[#fdfaf6] border border-[#d68636] flex items-center justify-center text-[#d68636]">
          <User className="w-4 h-4" />
        </div>
        <h3 className="text-[18px] md:text-[20px] font-bold text-[#5c1a1f]">{title}</h3>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
        <div className="flex flex-col gap-1.5">
          <label className="text-[14px] font-bold text-[#5c1a1f] mb-1">Full Name <span className="text-red-500">*</span></label>
          <input required type="text" value={data.name} onChange={(e) => setData({ ...data, name: e.target.value })} className="w-full border border-[#ebdcc7] rounded-md py-2.5 px-3 bg-[#fdfaf6] outline-none focus:border-[#d68636] text-[14px]" placeholder="Enter full name" />
        </div>
        ...
        <div className="flex flex-col gap-1.5 relative">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full border border-[#ebdcc7] rounded-md py-2.5 pl-9 pr-3 bg-[#fdfaf6] outline-none focus:border-[#d68636] text-[14px]" 
              placeholder="Enter place of birth (City)" 
              autoComplete="off"
            />
            {isSearching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 animate-spin text-[#d68636]" />}
          </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input required type="date" value={data.dob} onChange={(e) => setData({ ...data, dob: e.target.value })} onClick={(e) => { try { (e.target as HTMLInputElement).showPicker(); } catch (err) {} }} className="w-full border border-[#ebdcc7] rounded-md py-2.5 px-3 bg-[#fdfaf6] outline-none focus:border-[#d68636] text-[14px]" />
        </div>

        <div className="flex flex-col gap-1.5">
          <label className="text-[14px] font-bold text-[#5c1a1f] mb-1">Time of Birth <span className="text-red-500">*</span></label>
          <div className="relative">
            <input required={!unknownTime} disabled={unknownTime} type="time" value={data.tob} onChange={(e) => setData({ ...data, tob: e.target.value })} onClick={(e) => { try { (e.target as HTMLInputElement).showPicker(); } catch (err) {} }} className={`w-full border border-[#ebdcc7] rounded-md py-2.5 px-3 bg-[#fdfaf6] outline-none focus:border-[#d68636] text-[14px] ${unknownTime ? 'opacity-50' : ''}`} />
          </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {!isPartner && <p className="text-[11px] text-[#3a1216]/80 italic mt-0.5">Your Kundli report will be sent on this number</p>}
        </div>
      ...
      <div className="max-w-[900px] mx-auto px-4 pt-2 pb-8 md:pt-4 md:pb-12 space-y-6">
        ...
        <div className="bg-white rounded-xl border-[1.5px] border-[#c57636] p-5 md:p-8 shadow-md">
          ...
          <form id="checkout-form" onSubmit={handleFormSubmit}>
            ...
            {renderPersonForm("Boy's Details", boyData, setBoyData, boyUnknownTime, setBoyUnknownTime, showBoySuggestions, setShowBoySuggestions, boyLocationSuggestions, isSearchingBoyLocation, false)}
            
            {renderPersonForm("Girl's Details", girlData, setGirlData, girlUnknownTime, setGirlUnknownTime, showGirlSuggestions, setShowGirlSuggestions, girlLocationSuggestions, isSearchingGirlLocation, true)}
