...
async function fetchSettings() {
  try {
    const res = await fetch(`${API_URL}/smart-kundali-settings/fortune-numerology`, { next: { revalidate: 60 } });
...
export default async function FortuneNumerologyPage() {
  ...
  const defaultTestimonials = [
    {
      ...
      city: "Delhi",
      date: "December 2025",
      review: "The Fortune Numerology report completely changed my perspective. The remedies were so simple yet effective. My business has finally started picking up pace.",
      initial: "A",
      color: "#5c1a1f"
    },
    {
      ...
      <section className="relative w-full pt-6 pb-12 overflow-hidden bg-[#7a4b3a]">
        ...
        <div className="absolute inset-0 z-0 bg-[#4c2918]">
          {settings?.banner?.url && /\.(mp4|webm|mov)(\?.*)?$/i.test(settings.banner.url) ? (
            ...
            <img src={settings.banner.url} alt="Fortune Numerology Report" className="w-full h-full object-cover object-center opacity-80" />
          ) : (
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto px-4 text-center mt-2">
          <h1 className="text-[28px] md:text-[48px] lg:text-[58px] font-bold mb-3 font-serif leading-tight text-white drop-shadow-md">
            {settings?.heroHeading || "Fortune Numerology: Discover the Hidden Blueprint of Your Life"}
          </h1>
          <p className="text-[16px] md:text-[20px] text-white/100 mb-6 max-w-2xl mx-auto font-semibold drop-shadow-sm">
            {settings?.heroSubheading || "Unlock Your Potential with the Power of Numbers"}
          </p>
          ...
          <Link href="/report/numerology/fortune-numerology/checkout" className="inline-block bg-white text-[#b06126] font-bold text-[16px] md:text-[18px] px-10 py-3.5 md:py-4 rounded-xl shadow-lg hover:scale-105 transition-transform mb-6">
            {settings?.heroCtaText || "Get Your Numerology Report"} @ <span className="line-through text-[#3a1216] mx-1">₹{settings?.price || 600}</span> ₹{settings?.discountedPrice || 549}
          </Link>
      ...
      <section className="py-12 md:py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-20">
          ...
          <div className="hidden md:flex md:w-1/2 justify-center">
            <img src={settings?.mockups?.pdf || "/images/vaidiktalk-kundli-mockup.webp"} alt="Premium Fortune Numerology Report" className="w-full max-w-[470px] rounded-xl mix-blend-multiply" />
          </div>
          ...
          <div className="w-full md:w-1/2 flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-4 md:mb-3">
              <div className="md:hidden shrink-0 w-[85px] sm:w-[100px] flex items-center justify-center">
                <img src={settings?.mockups?.pdf || "/images/vaidiktalk-kundli-mockup.webp"} alt="Premium Fortune Numerology Report" className="w-full h-auto object-contain drop-shadow-md rounded-sm mix-blend-multiply" />
              </div>
              <h2 className="text-[22px] sm:text-3xl md:text-4xl font-serif font-bold text-[#5c1a1f] leading-tight">{settings?.productHeading || "Premium Fortune Numerology Report"}</h2>
            </div>
          ...
          <div className="md:w-[55%] lg:w-[60%] md:pl-6 lg:pl-10">
            ...
            <div className="space-y-6 mb-12">
              {(settings?.whatItReveals?.length ? settings.whatItReveals : [
                'Your most suitable numbers based on your birth date and name',
                'Deep analysis of your Life Path and Destiny Numbers',
                'Warnings about numbers you should avoid in business or life',
                'Guidance on name correction and mobile number numerology',
                'Simple household and charity-based remedies to balance karma'
            ...
            <a href="/report/numerology/fortune-numerology/checkout" className="inline-block bg-[#d68636] text-white font-bold text-[16px] px-10 py-4 rounded-xl shadow-md hover:bg-[#b06126] transition-colors w-full md:w-auto text-center">
              {settings?.videoSectionCtaText || "Order Your Numerology Report →"}
            </a>
          </div>
        </div>
      </section>
    ...
    <div key={i} className="w-[280px] sm:w-[320px] md:w-[360px] lg:w-[400px] aspect-video bg-black rounded-2xl overflow-hidden relative shadow-xl snap-center flex-shrink-0 border-[3px] border-white">
      ...
      <section className="py-16 md:py-24 bg-[#fdfaf6] text-white">
        <div className="max-w-[800px] mx-auto px-4 text-center">
          ...
          <p className="text-center text-[#3a1216]/80 text-[15px] md:text-[16px] mb-10 max-w-[600px] mx-auto">Get your personalized Fortune Numerology Report today and remove the blocks holding you back.</p>
          <Link href="/report/numerology/fortune-numerology/checkout" className="inline-block bg-[#d68636] text-white font-bold text-[18px] px-12 py-4 rounded-xl shadow-[0_8px_20px_rgba(214,134,54,0.3)] hover:bg-[#b06126] transition-all hover:scale-105">
            {settings?.bottomCtaText || "Book Numerology Report"}
          </Link>
        </div>
      </section>
