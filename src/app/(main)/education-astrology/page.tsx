...
import EducationAstrologyClient from './EducationAstrologyClient';

export const revalidate = 60; // ISR cache for 60 seconds


export async function generateMetadata(): Promise<Metadata> {
  const slug = 'education-astrology';
  const defaultMeta = {
    title: "Education Astrology & Studies | AstroSolution",
    description: "Find out the best field of study and overcome educational hurdles with Vedic astrology predictions.",
  };
...
async function fetchInitialData() {
  try {
    ...
    const [astrosRes, aiAstrosRes, settingsRes] = await Promise.all([
      ...
      fetch(`${API_URL}/smart-kundali-settings/education-astrology`, { next: { revalidate: 3600 } })
    ]);
    ...
    console.error('Error fetching SSR data for education-astrology:', error);
...
export default async function Page() {
  ...
  return (
    <>
      <PageSeoProvider slug="education-astrology" />
      <EducationAstrologyClient 
