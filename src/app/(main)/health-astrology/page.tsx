...
import HealthAstrologyClient from './HealthAstrologyClient';
...
async function fetchInitialData() {
  try {
    ...
    const [astrosRes, aiAstrosRes, settingsRes] = await Promise.all([
      ...
      fetch(`${API_URL}/smart-kundali-settings/health-astrology`, { next: { revalidate: 3600 } })
    ]);
    ...
    console.error('Error fetching SSR data for health-astrology:', error);
...
export default async function Page() {
  ...
  return (
    <>
      <PageSeoProvider slug="health-astrology" />
      <HealthAstrologyClient 
