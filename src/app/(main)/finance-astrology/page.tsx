...
import FinanceAstrologyClient from './FinanceAstrologyClient';
...
async function fetchInitialData() {
  try {
    ...
    const [astrosRes, aiAstrosRes, settingsRes] = await Promise.all([
      ...
      fetch(`${API_URL}/smart-kundali-settings/finance-astrology`, { next: { revalidate: 3600 } })
    ]);
    ...
    console.error('Error fetching SSR data for finance-astrology:', error);
...
export default async function Page() {
  ...
  return (
    <>
      <PageSeoProvider slug="finance-astrology" />
      <FinanceAstrologyClient 
