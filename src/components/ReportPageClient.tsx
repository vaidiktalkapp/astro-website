...
// ─── FaqSection ──────────────────────────────────────────────────────────────
export function FaqSection({ faqs, accentColor = '#d68636' }: { faqs: FaqItem[]; accentColor?: string }) {
          ...
          {faqs.map((faq, i) => (
            <div key={i} className="border border-[#ebdcc7] rounded-xl overflow-hidden bg-[#fdfaf6]">
              ...
              >
                ...
                {openFaq === i
                  ? <Minus className="w-5 h-5 flex-shrink-0" style={{ color: accentColor }} />
                  : <Plus className="w-5 h-5 flex-shrink-0" style={{ color: accentColor }} />}
              </button>
              {openFaq === i && (
    ...
    <>
      ...
      <div className="flex gap-3 md:gap-4 overflow-x-auto hide-scroll snap-x snap-mandatory pb-4">
        {screenshots.map((s, i) => (
          <div
            key={i}
            className="shrink-0 w-[200px] sm:w-[240px] md:w-[280px] lg:w-[300px] aspect-[3/4] rounded-2xl overflow-hidden shadow-xl snap-center cursor-pointer border-[3px] border-white hover:scale-[1.02] transition-transform"
